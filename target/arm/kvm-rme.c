...
static int rme_init_ram(hwaddr base, size_t size, Error **errp)
{
    ...
    struct kvm_cap_arm_rme_init_ipa_args init_args = {
        .init_ipa_base = QEMU_ALIGN_DOWN(base, RME_PAGE_SIZE),
        .init_ipa_size = QEMU_ALIGN_UP(size, RME_PAGE_SIZE),
    };
                              ...
                              Error **errp)
{
    ...
    struct kvm_cap_arm_rme_populate_realm_args populate_args = {
        .populate_ipa_base = QEMU_ALIGN_DOWN(base, RME_PAGE_SIZE),
        .populate_ipa_size = QEMU_ALIGN_UP(size, RME_PAGE_SIZE),
...
static void rme_rom_load_notify(Notifier *notifier, void *data)
{
    ...
    region->size = rom->len;
    // TODO: double-check that data is still available when we measure it, while
    // writing the log. Should be fine because data is kept for the next reset.
    region->data = rom->data;

    /*
     * rme_guest->images is destroyed after ram_regions, so we can store this
     * pointer even if we don't own the struct.
     */
