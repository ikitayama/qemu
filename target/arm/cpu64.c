                                ...
                                void *opaque, Error **errp)
{
    uint8_t val;
    ARMCPU *cpu = ARM_CPU(obj);
    uint8_t max_wps = FIELD_EX64(cpu->isar.id_aa64dfr0, ID_AA64DFR0, WRPS);

    if (!visit_type_uint8(v, name, &val, errp)) {
        return;
    }

    if (val < 2 || val > max_wps + 1) {
                                ...
                                void *opaque, Error **errp)
{
    uint8_t val;
    ARMCPU *cpu = ARM_CPU(obj);
    uint8_t max_bps = FIELD_EX64(cpu->isar.id_aa64dfr0, ID_AA64DFR0, BRPS);

    if (!visit_type_uint8(v, name, &val, errp)) {
        return;
    }

    if (val < 2 || val > max_bps + 1) {
